...
// connect to DB
        ...
        $res = $db->query('SELECT url,pageid FROM webpage WHERE ignorespell=0 and site_id='.$id);
		...
		for($i=0;$i<count($result);$i++){
            ...
            $stmt->execute();
            $stmt->close();
			echo "Начало проверки страницы: ".$result[$i]['url']."\r\n";
			...
			echo "Проверка страницы  ".$result[$i]['url']." завершена. Ошибочных слов =".$ew."\r\n";
